...
double fassoc2dMF(double x, void *p) {
	...
	double alp, bet, tet, P, T, h, f, Rmax;
	f_params &params = *reinterpret_cast<f_params *>(p);
	Rmax = 3.0 * sqrt(4.0 * params.D * params.t) + params.a;
	...
	if (params.k < 1.0 / 0.0) { //This gives the association probability
		...
		tet = M_PI*x*x*x*(alp * alp + bet * bet);


		P = M_PI*x*Rmax*(j1(x*Rmax)*bet-params.k*j0(params.a*x)*y1(x*Rmax)-h*x*j1(x*params.a)*y1(x*Rmax))-2*params.k;

//		P = j0(x * params.a) * y1(x * params.a) - j1(x * params.a) * y0(x * params.a);
		T = 1.0;//(j0(x * params.r0) * bet - y0(x * params.r0) * alp) / tet;

		f = T * P * (1 - exp(-params.D * params.t * x * x))/tet;
		f = -4.0* f * params.rho * params.k;
...
//		f = (2.0 / M_PI) * T * P * exp(-params.D * params.t * x * x) / x;
		f = 0.0; //haven't checked the abs boundary case
	}
