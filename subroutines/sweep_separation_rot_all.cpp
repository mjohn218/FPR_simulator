...
void sweep_separation_rot_all(double deltat, int p1, Fullmol *bases, Complex *ind_com, int *ncross, int **cross_part, int **cross_int, int **cross_rxn, double **traj, double **prob, Parms plist, int *movestat, int *ihome, double *bindrad, double **trajR, double *M, int **Rlist) {
	...
	/*Since you are only selecting new positions for interfaces that have rotated and translated from COM, 
	 the ind_com should not change until physically updated at the end.
	 */
	...
	while (it < maxit) {
		...
		for (i = 0; i < ncross[p1]; i++) {
			...
			rotate(v, M, v2);
			//cout <<"Pre, post rot: "<<v[0]<<' '<<v[1]<<' '<<v[2]<<' '<<v2[0]<<' '<<v2[1]<<' '<<v2[2]<<endl;
			dx1 = x0 + v2[0] + traj[k1][0]; //+bases[p1].x[iind1];
			dy1 = y0 + v2[1] + traj[k1][1]; //+bases[p1].y[iind1];
			dz1 = z0 + v2[2] + traj[k1][2]; //+bases[p1].z[iind1];
			// cout <<"current COM: "<<p1<<' '<<k1<<' '<<ind_com[k1].xcom<<' '<<ind_com[k1].ycom<<' '<<ind_com[k1].zcom<<" loop: "<<it<<endl;
			//cout <<"current displace: "<<traj[k1][0]<<' '<<traj[k1][1]<<' '<<traj[k1][2]<<' '<<dx1-ind_com[k1].xcom<<' '<<dy1-ind_com[k1].ycom<<' '<<dz1-ind_com[k1].zcom<<endl;
			/*Now complex 2*/

			rotationEuler(trajR[k2][0], trajR[k2][1], trajR[k2][2], M2);
			reflect_traj_complex_rad_rot(p2, bases, ind_com, plist.xboxl, plist.yboxl, plist.zboxl, traj, M2);
			x02 = ind_com[k2].xcom;
			y02 = ind_com[k2].ycom;
			z02 = ind_com[k2].zcom;
			...
			rotate(v, M2, v2);
			//cout <<"Pre, post rot, complex 2: "<<v[0]<<' '<<v[1]<<' '<<v[2]<<' '<<v2[0]<<' '<<v2[1]<<' '<<v2[2]<<endl;
			dx2 = x02 + v2[0] + traj[k2][0];
			dy2 = y02 + v2[1] + traj[k2][1];
			dz2 = z02 + v2[2] + traj[k2][2];
			//cout <<"current COM: "<<p2<<' '<<k2<<' '<<ind_com[k2].xcom<<' '<<ind_com[k2].ycom<<' '<<ind_com[k2].zcom<<endl;
			//cout <<"current displace: "<<traj[k2][0]<<' '<<traj[k2][1]<<' '<<traj[k2][2]<<' '<<dx2-ind_com[k2].xcom<<' '<<dy2-ind_com[k2].ycom<<' '<<dz2-ind_com[k2].zcom<<endl;
		...
		if (flag == 1) {
			...
			reflect_traj_complex_rad_rot(p1, bases, ind_com, plist.xboxl, plist.yboxl, plist.zboxl, traj, M);
			x0 = ind_com[k1].xcom;
			y0 = ind_com[k1].ycom;
			z0 = ind_com[k1].zcom;

			//cout <<"overlap: "<<p1<<" noverlap: "<<t<<" p2: "<<olaplist[0]<<" Matrix "<<M[0]<<' '<<M[1]<<' '<<M[2]<<' '<<M[3]<<' '<<M[4]<<' '<<M[5]<<' '<<M[6]<<' '<<M[7]<<' '<<M[8]<<" loop: "<<it<<endl;
			for (j = 0; j < t; j++) {
				p2 = olaplist[j];
				k2 = bases[p2].mycomplex;
				if (p2 > p1) {
					...
					//cout <<"retraj p2: "<<p2<<" during p1's go: "<<p1<<" complex2: "<<k2<<endl;
		...
		if (it == maxit - 1) {
			if (k1 != k2)
				cout << "can't solve overlap: " << p1 << " max cross: " << ncross[p1] << " n overlap: " << tsave << " pro1: " << olaplist[0] << endl;
			flags = 1;
		}
