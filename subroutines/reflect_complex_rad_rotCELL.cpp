...
void reflect_complex_rad_rotCELL(int p1, Fullmol *bases, Complex *ind_com, double xboxl, double yboxl, double zboxl) {
	...
	/*For complexes stuck to the membrane, ignore extension below plane.*/
	if(ind_com[k].Dz==0)rad=0;
...
//				cout<<bases[mp].z[j]-ind_com[k].zcom<<endl;
				if (zchg > 0) {
					flag = 1;
					if (-zchg < ztot)
						ztot = -zchg;
...
//			cout<<"in: "<<ind_com[k].zcom<<endl;
			ind_com[k].zcom += 2.0 * ztot;
//			if (abs(ind_com[k].zcom)>zboxl/2){
//				cout<<"out: "<<ind_com[k].zcom<<endl;
//			}
			//update protein COM
		...
		/*ztot and ztot0 will always be different if rad does not equal zero*/
		//if (abs(ztot - ztot0) > tol)
		//			for (j = 0; j < bases[mp].ninterface; j++) {
		//				cout<<bases[mp].z[j]-ind_com[k].zcom<<endl;
		//			}
		//cout << "Different calculations of z beyond box! " << ztot0 << " final: " << ztot << " molnum:" << p1 << endl;
	}
		//	if (flagy == 1) {
		//
//		flag = 0;
