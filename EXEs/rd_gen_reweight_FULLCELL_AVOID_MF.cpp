/*
 ...
 algorithm with trajectory reweighting with mean-field (MF)
 approximation. Test for 2 types of particles (P0:lipid and
 P1:membrane recruitable protein). In numfile.inp, we write
 the numbers of each species as usual (NP0 and NP1), but the
 program just does not create NP0 species to save memory.
 The membrane recruitment reactions are handled through MF
 approximation.
...
int main(int argc, char *argv[]) {
	 ...
	 */
	int i, idd, j, n, k, recruitmentflag,pskip;
	int nmolecinside;
	int *nlistspecies = new int[7];
	pskip = 0;
	recruitmentflag = 0;
	timeval tim;
	gettimeofday(&tim, 0);
	...
	double TBLID[MAXALWTBL][2]; //[type#][0]:ka, [tNx * Ny * (k + 1)ype#][1]:Dtot
	double Dtemp;
	double ktemp;
	int DDtableindex = 0;
	...
	for (i = 0; i < Nprotypes; i++) {
		if (i>0){
			...
			numfile >> d0;
			density0 = d0/(plist.xboxl * plist.yboxl);
			mydensity = density0;
			occupiedlipid = 0;
			Ncopy[i] = 0; //Assuming mol0 is lipid with high conc.
		}
		Ntotalmol += Ncopy[i];
	}
...
//	ofstream restart(tname);
	...
	int Nz = max(2,int(floor(plist.zboxl / Rmax2))); //4; //int(ceil(box_z/cellz));
	int Ncell = Nx * Ny * Nz;
	...
	ofstream disassemblyfile(tname2, std::ios::app);
		...
		for (i = 0; i < Ntotalmol; i++) {
			...
			for (j = 0; j < bases[i].nbnd; j++) {
				...
				for (n = 0; n < Nmyrxn[icom]; n++) {
					...
					if (i < ppart || ppart == -1) {
						/*then this is the first protein in the reaction, so try it. TRY IT IF IT'S A MEMBRANE
...
//								assemblyfile<<-1<<'\t'<<it*deltat<<'\t'<<i<<'\t'<<ppart<< endl;
								...
								if (ppart == -1){
									cancel = break_complexPBCCELLMEM(i, mu, j, bases, Rlist, i_home, ind_com, plist, bindrad, ppart, i1, i2, p_home, myrxn,wholep);
									...
									cancel = break_complexPBCCELLMF(i, mu, j, bases, Rlist, i_home, ind_com, plist, bindrad, ppart, i1, i2, p_home, myrxn, wholep);
		...
		get_distancePBCCELLMEM(numpartners, Speclist, myrxn, mydensity,wholep, ind_com, kr, bases, deltat, bindrad, ncross, crosspart, probvec, crossint, cross_rxn, plist);
...
//											dz -= plist.zboxl * round(dz / plist.zboxl);
											...
											if (ind_com[myc1].Dz == 0 && ind_com[myc2].Dz == 0) { // This is a reaction on the membrane
												...
												nc1 = ncross[i];
												nc2 = ncross[j];
												flagsep = get_distancePBCCELL(bases, traj, i, j, deltat, bindrad[mu], ncross, crosspart, crossint, i1, i2, mu, cross_rxn, i_home, it, Rmax, sep, R1, plist, ncross2);
		...
		for (i = 0; i < Ntotalmol; i++) {
			...
			if (ncross[i] > 0) {
				...
				if (flag == 1) {
					...
					if (p2==-1){
						cout << "Associate protein: p1 " << p1 << " on membrane lipid with interface: " << crossint[p1][ci1] << " reaction: " << rxn1 << " pact; " << probvec[p1][ci1] << " rnum: " << rnum << " iter: " << it << endl;
						occupiedlipid++;
						associate_translate_measurePBCCELLMEM(p1, p2, rxn1, crossint[p1][ci1], bases, Rlist, i_home, ind_com, plist, wholep);
							...
							if (pskip != p2) {
								...
								for (j = 0; j < ncross[pskip]; j++) {
									if (crosspart[pskip][j] == p1) {
										...
										probvec[pskip][j] = probvec[pskip][ncross[pskip] - 1];
										cross_rxn[pskip][j] = cross_rxn[pskip][ncross[pskip] - 1];
										crossint[pskip][j] = crossint[pskip][ncross[pskip] - 1];
										ncross[pskip] -= 1;
										j -= 1;
	...
	//					cout << bases[p2].xcom << ' ' << bases[p2].ycom << ' ' << bases[p2].zcom <<endl;
						...
						if (bases[p1].protype == bases[p2].protype) {
							...
							if (bases[p1].protype == plist.pclath)
								...
								associate_translate_measurePBCCELLMF(p1, p2, rxn1, crossint[p1][ci1], crossint[p2][ci2], bases, Rlist, i_home, ind_com, plist,wholep);
						} else {
	    ...
	    mydensity = (d0-occupiedlipid)/(plist.xboxl * plist.yboxl);
		...
		if (it % statwrite == 0) {
			...
			cout<<"Current Density:"<<mydensity<<endl;
		}
		if (it % twrite == 0) {
			ofstream restart(tname);
...
//			write_timestat(timestatfile, wholep, bases, plist, Ncopy, it + iterass, ind_com, deltat, Nprotypes); ///was assemblyfile
			...
			nlistspecies[0] = d0-occupiedlipid;
			for (i = 0; i < plist.ntotalcomplex; i++) {
				...
				if (nmolecinside == 1) {
					...
					} else if (p1 < (Ncopy[1] + Ncopy[2])) {
						if (bases[p1].nbnd>0){
							nlistspecies[5] += 1; //EPN1-PIP2
						}else{
							nlistspecies[2] += 1; //EPN
						}
					}
				} else if (nmolecinside == 2) {
					p1 = ind_com[i].plist[nmolecinside - 2];
					p2 = ind_com[i].plist[nmolecinside - 1];
					if ((bases[p1].nbnd+bases[p2].nbnd)==2){
						nlistspecies[3] += 1; //AP2-EPS
					}else if ((bases[p1].nbnd+bases[p2].nbnd)>=3){
						nlistspecies[6] += 1; //MEM-Complex
					}
				}

			}

			timestatfile << it * deltat * 1e-6 << "\t" << nlistspecies[0] << "\t" << nlistspecies[1] << "\t" << nlistspecies[2] << "\t" << nlistspecies[3] << "\t" << nlistspecies[4] << "\t" << nlistspecies[5] << "\t" << nlistspecies[6] << endl;
//			timestatfile << (it + iterass) * deltat * 1e-6 << "\t" << d0-occupiedlipid << Ncopy[1]-occupiedlipid <<endl;
		}
