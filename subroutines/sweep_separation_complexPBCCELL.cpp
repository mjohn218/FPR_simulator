...
void sweep_separation_complexPBCCELL(double deltat, int p1, Fullmol *bases, Complex *ind_com, int *ncross, int **cross_part, int **cross_int, int **cross_rxn, double **traj, double **prob, Parms plist, int *movestat, int *ihome, double *bindrad, int **Rlist) {
	/*
	...
	while (it < maxit) {
		...
		if (flag == 1) {
			...
			reflect_traj_complex_radCELL(p1, bases, ind_com, plist.xboxl, plist.yboxl, plist.zboxl, traj);
			...
			for (j = 0; j < t; j++) {
				...
				if (p2 > pstart && movestat[p2] != 2) {
					...
					reflect_traj_complex_radCELL(p2, bases, ind_com, plist.xboxl, plist.yboxl, plist.zboxl, traj);
		...
		if (it == maxit - 1) {
			...
			// for(c=0;c<csize;c++){
			// 	p1=ind_com[k1].plist[c];
			// 	cout <<"p1: "<<p1<<' '<<bases[p1].xcom<<' '<<bases[p1].ycom<<' '<<bases[p1].zcom<<endl;
			//       }
