...
//   double limit.kmat_ps=limit.limit.kmat_ps;
  ...
  double dens=exp(-rc/Rmax1);
  cout <<"density at Rmax: "<<dens<<endl;
      ...
      for(it=1;it<Nit+1;it++)
	{
	  ...
	  for(c=0;c<Ncell;c++){
		...
		if(flagsep==1){
		  ...
		  place2=i*Ncopy[0]+j;
		  if(R1<limit.fit1){
		    probvec1=survive_irr(R1, deltat, limit.Dmat_ps[0], bindrad[0], alpha1, cof1);
		  }else{
		    probvec1=survive_irr(R1, deltat, limit.Dmat_ps[1], bindrad[0], alpha2, cof2);
		  }
		...
		for(qq=0;qq<npb[nb];qq++){
		  ...
		  if(flagsep==1){
		    ...
		    if(sep<0){
		      ...
		      // for(int s=0;s<Ntotalmol;s++){
		      // 	cout <<bases[s].xcom<<' '<<bases[s].ycom<<' '<<bases[s].zcom<<endl;
		      // }
		      sep=0;
		      ratio=1;
		      R1=bindrad[mu];
...
// 		bases[p2].zcom=bases[p1].zcom;
		...
		*/
		if(movestat[i]==0){
		  ...
		  Fy=0;
		  Fz=0;
		  for(nc=0;nc<ncross[i];nc++){
		    //sum up forces on this particle
		    //can't use xcom, have to use dx=1-0.
		    p2=crosspart[i][nc];
		    myplace=i*Ncopy[0]+p2;
		    R1=prevsep[myplace];
		    R2=R1*R1;
		    dudr=-rc/(R2);
		    dx=bases[i].xcom-bases[p2].xcom;
		    dy=bases[i].ycom-bases[p2].ycom;
		    dz=bases[i].zcom-bases[p2].zcom;
		    ...
		    Fy+=-dudr*dy/R1;
		    Fz+=-dudr*dz/R1;
		    //cout <<"Rep: "<<rep<<" it: "<<it<<" Pair: "<<i<<'\t'<<p2<<" sep: "<<R1<<" Fx: "<<Fx<<" Fy: "<<Fy<<" Fz "<<Fz<<" Ftot: "<<-dudr<<endl;
		  }
