...
void sweep_separation_all(double deltat, int p1, Fullmol *bases, Complex *ind_com, int *ncross, int **cross_part, int **cross_int, int **cross_rxn, double **traj, double **prob, Parms plist, int *movestat, int *ihome, double *bindrad, int **Rlist) {
	...
	while (it < maxit) {
		...
		if (flag == 1) {
			...
			reflect_traj_complex_rad(p1, bases, ind_com, plist.xboxl, plist.yboxl, plist.zboxl, traj);
			for (j = 0; j < t; j++) {
				p2 = olaplist[j];
				k2 = bases[p2].mycomplex;
				if (p2 > p1) {
					...
					reflect_traj_complex_rad(p2, bases, ind_com, plist.xboxl, plist.yboxl, plist.zboxl, traj);
				}
			}
			tsave = t;
		} else {
	...
	/*
	 ...
	 //used this to push them apart, but binding radius will be for 
	 //last value of "rxn" index.
	 dx1=traj[k1][0]+bases[p1].x[iind1];
	 dy1=traj[k1][1]+bases[p1].y[iind1];
	 dz1=traj[k1][2]+bases[p1].z[iind1];
