...
int break_complexPBCCELLMF(int p1, int mu, int kind, Fullmol *bases, int **Rlist, int *ihome, Complex *ind_com, Parms &plist, double *bindrad, int p2, int i1, int i2, int *p_home, int **myrxn,Protein *wholep) {
	...
	if (cancel == 0) {
		 ...
		 */
		update_diffusionMF(c1, ind_com, bases, wholep);
		update_diffusionMF(c2, ind_com, bases, wholep);
...
//		dz -= plist.zboxl * round(dz / plist.zboxl);
		...
		move_pro_coordsPBCCELL(c2, ind_com, bases, chg2, plist);

		update_one_com_onlyPBCCELL(c1, ind_com, bases, plist, flagbndry);
		update_one_com_onlyPBCCELL(c2, ind_com, bases, plist, flagbndry);

		update_radiusPBCCELL(c1, ind_com, bases, plist);
		update_radiusPBCCELL(c2, ind_com, bases, plist);
