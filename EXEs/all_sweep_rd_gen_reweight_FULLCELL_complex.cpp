...
int main(int argc, char *argv[]) {
	 ...
	 */
	int i, idd, j, n, k, recruitmentflag, neighborcounter, k1, jj, existflag, overlaps=0, ncrossxx;
	int dd=0;
	int maxislandsize, islandnum;
	double meanislandsize, dr2, ncrossxxd;
	recruitmentflag = 0;
	timeval tim;
	gettimeofday(&tim, 0);
...
//		Ntotalmol += Ncopy[i];
	...
	int *islandvec = new int[Ntotalmol];
	int *islandsize = new int[Ntotalmol+1];
...
//	ofstream restart(tname);
	...
	int **neighbors = new int*[Ntotalmol];
	for (i = 0; i < Ntotalmol; i++) {
		...
		neighbors[i] = new int[newsizeoverlap];
	}
	...
	ofstream disassemblyfile(tname2, std::ios::out);
...
//    DDmatrixcreate(msur2, mnorm2, mpir2, bindrad[2], Dtot, kr[2]/2/bindrad[2], deltat);
	...
	for (it = 1; it < Nit + 1; it++) {
	  ...
	  */
//	  //check for ncross stats
//	  ncrossxx = 0;
//	  ncrossxxd = 0.0;
//	  for(i=0;i<Ntotalmol;i++){
//		  ncrossxx += ncross[i];
//		  if (it % twrite == 0) {
//			  disassemblyfile<<ncross[i]<<endl;
//		  }
//	  }
//	  ncrossxxd = (ncrossxx*1.0)/(Ntotalmol*1.0);
	  
	  for (i = 0; i < Ntotalmol; i++) {
	    /*Skip any proteins that just dissociated during this time step*/

		  //find unique molecule crosses and the molecules from the cross-complexes for each molecule
		  neighborcounter = 0; // Find all the neighbors of i for overlap solution
		  k1 = bases[i].mycomplex;
		  for(jj=0;jj<ind_com[k1].mysize;jj++){ // put all i's complex members into neighbors[]
			  if(ind_com[k1].plist[jj]!=i){
				  neighborcounter++;
				  neighbors[i][neighborcounter] = ind_com[k1].plist[jj];
			  }
		  }

		  for(j=0;j<ncross[i];j++){
			  existflag = 0;

			  for(k=1;k<neighborcounter+1;k++){// check if we already have j in i's neighbor list
				  if(neighbors[i][k]==crosspart[i][j]){
					  existflag = 1;
				  }
			  }
			  if(existflag==0){ //i and j can react
				  k1 = bases[crosspart[i][j]].mycomplex; //j's complex

				  for(jj=0;jj<ind_com[k1].mysize;jj++){ // put all j's complex members into neighbors[]
					  neighborcounter++;
					  neighbors[i][neighborcounter] = ind_com[k1].plist[jj];
				  }
			  }
		  ...
		  neighbors[i][0] = neighborcounter; // number of neighbors of i
	  ...
	  islander(movestat, islandsize, islandvec, newsizeoverlap, Ntotalmol, bases, ind_com, ncross, crosspart, neighbors, maxislandsize, meanislandsize, islandnum);
	  overlaps=0;
	  ...
	  for (i = 1; i <= islandnum; i++) {
	    ...
	    /*
	      ...
	      performing final position updates. movestat[i] != 2 && no neighbor molecules are with islandnum 0, and won't move at all.
	    */

		  if (islandsize[i]>1) { //or if ncrosscom[bases[i].mycomplex] > 0
		  ...
		  Also, if both proteins are stuck to membrane, only do xy displacement, ignore z
			  ...
			  all_sweep_separation_complex_rot_memtest_PBCCELL(overlaps, islandsize[i], neighbors, i, Ntotalmol, islandvec, deltat, bases, ind_com, ncross, crosspart, crossint, cross_rxn, traj, probvec, plist, movestat, i_home, bindrad, trajR, M, Rlist);
...
//				  sweep_separation_complex_rot_PBCCELL(deltat, i, bases, ind_com, ncross, crosspart, crossint, cross_rxn, traj, probvec, plist, movestat, i_home, bindrad, trajR, M, Rlist);
		  ...
		  } else{// means islandsize==1 loners//if islandsize is 1 old if (ncrosscom[bases[i].mycomplex] == 0)
		  ...
		  were selected based on the complex they were part of, and they may not yet been moved.
			   */
				for(j = 0; j < Ntotalmol; j++){
					if(islandvec[j]==i){
						  move_rot_proteinsPBCCELL(j, bases, ind_com, traj, movestat, trajR, M, plist);
						  reflect_complex_rad_rotCELL(j, bases, ind_com, plist.xboxl, plist.yboxl, plist.zboxl);
						  j = Ntotalmol; //break out of the loop
...
// Test to see if anybody is overlapping
//	  for(i=0;i<Ntotalmol-1;i++){
//		  for(j=i+1;j<Ntotalmol;j++){
//			  t = check_overlap(dr2, i, j, bases, ind_com, ncross, crosspart, crossint, cross_rxn, traj, plist, i_home, bindrad, trajR, M, Rlist);
//			  if(t>0){
//				  cout<<i<<"\t"<<j<<"\t"<<dr2<<endl;
//			  }
//		  }
//	  }
	  ...
	  if (it % twrite == 0 && it % gwrite != 0) {//timestat file written here
	  ...
	  if (it % gwrite == 0) {//restart file written here
	    ...
	    write_restart(restart, wholep, bases, plist, Ncopy, it + iterass, ind_com, deltat);
	    restart.close();
	    write_timestat(timestatfile, wholep, bases, plist, Ncopy, it + iterass, ind_com, deltat, Nprotypes); //so that timestat and restart files go in sync
	  }
	...
	disassemblyfile << timer_duration(totaltime) <<endl;
	assemblyfile.close();
