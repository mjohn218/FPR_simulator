...
void generate_initial_crds_AB_allavoid(Parms plist, Fullmol *bases, int *Ncopy, Complex *ind_com, double *bindrad) {
	...
	for (i = 0; i < plist.Ntotalmol; i++) {
		...
		if (bases[i].Dz == 0)
			bases[i].zcom = 0.0; //plist.zboxl*rand_gsl()-plist.zboxl/2.0;
		else
	...
	while (bflag == 1 && bit < Maxit) {
		...
		noverlap = 0;
		for (i = 0; i < plist.Ntotalmol; i++) {
			/*THE DIFFERENCE TO THE ONE BELOW, all particles avoid overlap    */
			for (j = i + 1; j < plist.Ntotalmol; j++) { //for(j=Ncopy[0];j<plist.Ntotalmol;j++)
				...
				if (d2 < pbindr2) {
					...
					if (bases[j].Dz == 0)
						bases[j].zcom = 0.0; //plist.zboxl*rand_gsl()-plist.zboxl/2.0;
					else
