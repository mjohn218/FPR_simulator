...
//     else nperdec[i]=100;
  ...
  */
  ...
  passoc=survive_irr_abs(bindrad[0], deltat, Dtot, bindrad[0]);
  cout <<bindrad[0]<<'\t'<<passoc<<endl;
  int loop=0;
  i=0;
  while(loop==0){
    r0=(i+0.5)*0.001+bindrad[0];
    passoc=survive_irr_abs(r0, deltat, Dtot, bindrad[0]);
    cout <<r0<<'\t'<<passoc<<endl;
    if(passoc<1E-13){
		...
		if(flagsep==1){
		  ...
		  for(s=0;s<nprevpart[proa];s++){
		    if(prevlist[proa][s]==pro2){
		      p0_ratio=pirr_abs_pfree_ratio_ps(R1, prevsep[proa][s], deltat, Dtot, bindrad[mu], ps_prev[proa][s], rtol);
