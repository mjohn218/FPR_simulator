...
//  }
  ...
  */
  ...
  double Dtot=2.0*wholep[0].Dx;//+wholep[1].Dx;//This is for Dtot between protein 0 and protein 1
  double deltat=plist.deltat;
  double Rmax1=3.0*sqrt(4.0*Dtot*deltat)+bindrad[0];//bindrad for reaction 0
...
//  }
  ...
  for(rep=0;rep<Nrep;rep++){
    ...
    generate_initial_crds_AB_allavoid( plist, bases, Ncopy, ind_com, bindrad);//all molecules will avoid each other
