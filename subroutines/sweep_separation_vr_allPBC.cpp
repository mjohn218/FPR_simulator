...
void sweep_separation_vr_allPBC(double deltat, int p1, Fullmol *bases, Complex *ind_com, int *ncross, int **cross_part, int **cross_int, int **cross_rxn, double **traj, double **prob, Parms plist, int *movestat, int *ihome, double *bindrad, int **Rlist, double **Ftraj) {
	...
	while (it < maxit) {
		...
		for (i = 0; i < ncross[p1]; i++) {
			...
			iind1 = ihome[i1];
			iind2 = ihome[i2];
			dx1 = traj[k1][0] + bases[p1].x[iind1] + Ftraj[k1][0];
			dy1 = traj[k1][1] + bases[p1].y[iind1] + Ftraj[k1][1];
			dz1 = traj[k1][2] + bases[p1].z[iind1] + Ftraj[k1][2];

			dx2 = traj[k2][0] + bases[p2].x[iind2] + Ftraj[k2][0];
			dy2 = traj[k2][1] + bases[p2].y[iind2] + Ftraj[k2][1];
			dz2 = traj[k2][2] + bases[p2].z[iind2] + Ftraj[k2][2];
...
//       cout <<bases[olaplist[0]].xcom<<' '<<bases[olaplist[0]].ycom<<' '<<bases[olaplist[0]].zcom<<endl;
	...
	dx1 = traj[k1][0] + Ftraj[k1][0];
	dy1 = traj[k1][1] + Ftraj[k1][1];
	dz1 = traj[k1][2] + Ftraj[k1][2];
	int mp;
	ind_com[k1].xcom += dx1;
	ind_com[k1].ycom += dy1;
	ind_com[k1].zcom += dz1;
