...
int main(int argc, char *argv[])
{
  ...
  */
  ...
  cout <<"GENERATING COORDS FOR AB INTERACTIONS! "<<endl;
  generate_initial_crds_ABavoid( plist, bases, Ncopy, ind_com, bindrad);
  ...
  */
  ...
  double Dtot2D=wholep[0].Dx+wholep[1].Dx;//This is for Dtot between protein 0 and protein 1
  double Rmax2D=3.0*sqrt(4.0*Dtot2D*deltat)+bindrad[0];

  int rnumsample, veclen;

  rnumsample = 99+5*floor(Rmax2D);
  ...
  TBLpirr(mpir, veclen, bindrad[0], Dtot2D, kr[0], deltat, Rmax2D, RstepSize);
	...
	for(pp=0;pp<npb[c];pp++){
	  ...
	  if(nfree>0){
		...
		if(wholep[wprot].propart[n]==bases[j].protype){
	      ...
	      //cout <<"protein i: "<<i<<" j: "<<j<<" flag: "<<flag<<endl;
	      if(flag==1){
		...
		*/
		...
		for(p=0;p<bases[i].nfree;p++){
		  ...
		  np=numpartners[i1];
		  //cout <<"i1: "<<i1<<" numpartners: "<<np<<endl;
		  for(n=0;n<np;n++){
...
// 			write_crds(bases, j);
			...
			//cout <<" p1: "<<i<<" p2: "<<j<<" distance: "<<R1<<" Dtot: "<<Dtot<<" Rmax: "<<Rmax<<endl;
			if(flagsep==1){
			  ...
			  */
			  if(myc1!=myc2){
				...
				p0_ratio = pirr_pfree_ratio_ps2D(R1, Dtot, deltat, prevsep[proa][s], pirrval, pnormval, ps_prev[proa][s], rtol);
				currnorm=prevnorm[proa][s]*p0_ratio;
				
				ssave=s;
				//if(proa==1377)
				//cout <<" iter: "<<it<<" found p1: "<<proa<<" prevpartner: "<<pro2<<" myface: "<<myface<<" hist: "<<pface<<" ps_prev: "<<ps_prev[proa][s]<<" prevsep: "<<prevsep[proa][s]<<" nprevpartner: "<<nprevpart[proa]<<'\t'<<" prevnorm: "<<prevnorm[proa][s]<<" currnorm: "<<currnorm<<'\t'<<endl;
				s=nprevpart[proa];
			      }
			    }
			  ...
			  //if(proa==1377)
			  //cout <<" iter: "<<it<<" same box "<<"proa: "<<proa<<" s: "<<s<<" pro2: "<<pro2<<" currlist: "<<currlist[proa][s]<<" probvec: "<<probvec1<<" normfact: "<<currnorm<<" R: "<<R1<<" ps_prev: "<<ps_prev[proa][ssave]<<" iter: "<<it<<endl;
...
// 			  write_crds(bases, j);
			  ...
			  if(flagsep==1){
			    ...
			    */
				...
				//  if(proa==1377)
				//  cout<<" iter: "<<it<<" diff box " <<" found p1: "<<proa<<" prevpartner: "<<pro2<<" myface: "<<myface<<" hist: "<<pface<<" ps_prev: "<<ps_prev[proa][s]<<" prevsep: "<<prevsep[proa][s]<<" nprevpartner: "<<nprevpart[proa]<<'\t'<<" prevnorm: "<<prevnorm[proa][s]<<" currnorm: "<<currnorm<<'\t'<<endl;
				  ssave=s;
				  s=nprevpart[proa];
				}
			      }
...
// 	  }
	  ...
	  */
	  ...
	  if(flag==1){
		...
		associate_freelegPBC(p1,  p2, rxn1, crossint[p1][ci1], crossint[p2][ci2], bases, Rlist, i_home, ind_com, plist);
	      else
	  ...
	  */
	  if(movestat[i]==0){
	    ...
	    move_rot_proteinsPBC(i, bases, ind_com, traj, movestat, trajR, M, plist); 
	...
	*/
	...
	if(ncross[i]>0){
	  ...
	  sweep_separation_rot_allPBC(deltat, i, bases, ind_com, ncross, crosspart, crossint, cross_rxn, traj, probvec, plist, movestat, i_home, bindrad, trajR, M, Rlist); 
	...
	Aavg[it]=Ncopy[0]-(Ncsave-plist.ntotalcomplex);
	cout<<"timestep: "<<it*deltat<<'\t'<<plist.ntotalcomplex<<" Nleg sites, if clathrin: "<<Nacurr<<" if single self: "<<Aavg[it]<<endl;
  ...
  for(i=0;i<Nit+1;i+=twrite){
    avfile << std::setprecision(8) <<i*deltat<<'\t'<<Aavg[i]/(1.0*(rep+1))<<endl;
  }
  avfile.close();
