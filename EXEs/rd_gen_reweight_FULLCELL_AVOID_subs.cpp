...
int main(int argc, char *argv[]) {
	 ...
	 */
	int i, idd, j, n, k, recruitmentflag;
	recruitmentflag = 0;
	timeval tim;
	gettimeofday(&tim, 0);
...
//    DDmatrixcreate(msur2, mnorm2, mpir2, bindrad[2], Dtot, kr[2]/2/bindrad[2], deltat);
	...
	for (it = 1; it < Nit + 1; it++) {
	  ...
	  /*Test dissociation!! */
	  dissociate_callPBC( Ntotalmol,  bases,  ind_com,  plist,  p_home,  i_home,  Rlist,  Nmyrxn, myrxn,  bindrad,  Ncoup,  mycoupled,  irandmax,  ndiss, disslist, ncross, kr, it);
	  ...
	  */
	  ...
	  for (c = 0; c < Ncell; c++) {
		...
		for (qq = pp + 1; qq < npb[c]; qq++) {
		  ...
		  evaluate_binding_pair(i, j, bases,  ind_com,  plist,  wholep,  DDtableindex,  numpartners,  Speclist,  i_home,  ncross,  probvec,  TBLID, bindrad,  nprevpart,  prevlist,  prevmyface, prevpface,  prevnorm,  ncurrpart,  currlist,  currmyface,  currpface,  ps_prev, prevsep,  currprevnorm,  currps_prev,  currprevsep, myrxn, traj, crosspart, crossint, cross_rxn, kr, it, contsur, contnorm, contpir, MAXALWTBL, ncrosscom);
	  ...
	  for (i = 0; i < Ntotalmol; i++) {
		...
		reaction is performed. Returns correct ci1 and ci2 for this rxn.
	      */
	      ...
	      flag = choose_one_reaction(rnum, i, ncross, crosspart, probvec, ci1, ci2, cross_rxn, crossint, irandmax);
	      
	      if (flag == 1) {
		
		perform_association(i,  bases,  ind_com, plist,  crosspart,  cross_rxn, crossint,  Rlist,  i_home,  probvec,  Ncoup,  mycoupled,  p_home,  ci1,  ci2,  it, movestat, ncross);
