...
double preflect_pfree_ratio(double rcurr, double r0, double tcurr, double Dtot, double bindrad, double alpharef, double rtol) {
	...
	double a2 = alpharef * alpharef; //just sqrt(D)/bindrad because kact=0
	...
	e1 = 2.0 * sep / sq_fDt * sqrt_t * alpharef + a2 * tcurr;
	ef1 = sep / sq_fDt + alpharef * sqrt_t;
	term2 = alpharef * exp(e1) * erfc(ef1);
