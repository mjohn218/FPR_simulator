...
void associate_freelegPBC(int p1, int p2, int mu, int i1, int i2, Fullmol *bases, int **Rlist, int *ihome, Complex *ind_com, Parms &plist) {
	...
	rotate_and_translate_intPBC(p1, c1, ind_com, bases, Mneg, dtrans, iind, plist);

	/*Update position of proteins in c2*/
	rotate_and_translate_intPBC(p2, c2, ind_com, bases, M, drev, iind2, plist);
	...
	rotate_onlyPBC(p1, c1, ind_com, bases, M2, plist);
	rotate_onlyPBC(p2, c2, ind_com, bases, M2neg, plist);
	...
	if (cancel == 0) {
		...
		if (c1 == c2) {
			...
			update_radiusPBC(c1, ind_com, bases, plist); //requires correct ind_com
		...
		rotate_onlyPBC(p1, c1, ind_com, bases, M2neg, plist);
		rotate_onlyPBC(p2, c2, ind_com, bases, M2, plist);
		...
		/*Update the position of proteins in complex one*/
		rotate_and_translate_intPBC(p1, c1, ind_com, bases, M, dtrans, iind, plist);

		/*Update position of proteins in c2*/
		rotate_and_translate_intPBC(p2, c2, ind_com, bases, Mneg, drev, iind2, plist);
