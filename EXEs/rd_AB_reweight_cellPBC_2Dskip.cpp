...
void gen_copy_coords_main(int Nprotypes, Protein *wholep, Fullmol* bases, Complex *ind_com, int *Ncopy);
...
//  }
  ...
  for(rep=0;rep<Nrep;rep++){
    ...
    generate_initial_crds_ABavoid( plist, bases, Ncopy, ind_com, bindrad, 0);
    //reset diffusion constants back to bases
    gen_copy_coords_main( Nprotypes, wholep,  bases,ind_com, Ncopy);
    write_complex(compout, plist, ind_com, 0);
    write_protein_iface_short(proout, plist, bases,Ncopy,0, wholep, names);
...
//    cout<<"timestep us: "<<it*deltat<<'\t'<<plist.ntotalcomplex<<'\t'<<Nacurr<<'\t'<<(Nacurr-Aeq)/A0mAeq<<endl;
	...
	for(j=0;j<bases[i].nbnd;j++){
      ...
      // int totalpercell=0;
//       for(i=0;i<Ncell;i++){
// 	cout <<"Npercell: "<<npb[i]<<endl;
// 	totalpercell+=npb[i];
//       }
//       cout <<"total: "<<totalpercell<<endl;
...
// 			p0_ratio = pirr_pfree_ratio_ps(R1, Dtot, deltat, prevsep[proa][s], pirrval, pnormval, ps_prev[proa][s], rtol);
			p0_ratio = DDpirr_pfree_ratio_ps(mpir, msur, mnorm, R1,  Dtot,  deltat, prevsep[proa][s],  ps_prev[proa][s],  rtol,  bindrad[mu]); //pirr_pfree_ratio_ps(R1, Dtot, deltat, prevsep[proa][s], pirrval, pnormval, ps_prev[proa][s], rtol);
	    ...
	    //cout <<"Iteration: "<<it<<'\t';
	    //for(int tmp=0;tmp<Ntotalmol;tmp++){
	    //cout <<"Ncross:" <<ncross[i]<<endl;
	    //}
