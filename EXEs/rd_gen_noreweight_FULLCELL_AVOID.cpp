...
//	ofstream restart(tname);
	...
	int myc1, myc2;
	int Nx = 10;//int(floor(plist.xboxl / Rmax1)); //10; //int(ceil(box_x/cellx));
	int Ny = 10;//int(floor(plist.yboxl / Rmax1)); //10; //int(ceil(box_y/celly));
...
//								assemblyfile<<-1<<'\t'<<it*deltat<<'\t'<<i<<'\t'<<ppart<< endl;
								...
								cancel = break_complexPBCCELL(i, mu, j, bases, Rlist, i_home, ind_com, plist, bindrad, ppart, i1, i2, p_home, myrxn);
...
//																	}
												...
												if (flagsep == 1 && kr[mu]>0) {
													...
													if (myc1 != myc2) {
														for (s = 0; s < nprevpart[proa]; s++) {
															if (prevlist[proa][s] == pro2 && prevmyface[proa][s] == myface && prevpface[proa][s] == pface) {
																p0_ratio = 1.0;//pirr_pfree_ratio_psF(R1, prevsep[proa][s], deltat, Dtot, bindrad[mu], alpha, ps_prev[proa][s], rtol);
